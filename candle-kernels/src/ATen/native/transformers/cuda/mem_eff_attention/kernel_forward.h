...
// Candle only instantiates the inference kernel with dropout disabled. These
// lightweight definitions preserve the upstream parameter fields and allow
// CUDA to discard the unreachable Philox path at compile time.
namespace at {
struct PhiloxCudaState {
  ...
  constexpr PhiloxCudaState(
      unsigned long long seed,
      unsigned long long offset)
      : seed_(seed), offset_(offset) {}
};
namespace cuda::philox {
struct PhiloxStateValues {
  unsigned long long seed;
  unsigned long long offset;
};
static __device__ __forceinline__ PhiloxStateValues unpack(
    PhiloxCudaState const& state) {
...
#define CHECK_ALIGNED_PTR(pointer, alignment) \
  assert(reinterpret_cast<uintptr_t>(pointer) % (alignment * sizeof(*(pointer))) == 0)
